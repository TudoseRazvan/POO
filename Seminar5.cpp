...
struct Pacient
{
	...
	int varsta;
	char sex;
	float* analize; //Pointer pentru alocare dinamica
};
...
int main()
{
	...
	struct Pacient pacient1;

	pacient1.analize = new float[numarAnalize];

	for (int i = 0; i < numarAnalize; i++) {
		pacient1.analize[i] = i * 1;
	}

	strcpy_s(pacient1.nume, "Ion Vasile");
	pacient1.varsta = 50;
	pacient1.sex = 'M';


	cout << "Nume: " << pacient1.nume << endl;
	cout << "Varsta: " << pacient1.varsta << endl;
	cout << "Sex: " << pacient1.sex << endl;

	cout << "Analize: ";
	for (int i = 0;i < numarAnalize;i++) {
		cout << pacient1.analize[i] << " ";
	}
	cout << endl;

	delete[] pacient1.analize;
